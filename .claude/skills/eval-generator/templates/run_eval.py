...
def main(pattern, limit, tags, model, temperature, version, as_json):
    """Run evaluation with assertions and streaming output."""
    ...
    if as_json:
        result = runner.run_batch(test_cases, workflow="eval", max_concurrent=5)
        click.echo(result.to_json())
    else:
        print_header(f"Evaluating {len(test_cases)} test(s)")
        print_config_info(runner.model, runner.temperature, runner.version)
        asyncio.run(run_streaming_loop(runner.astream_eval, test_cases, print_stream_event))
