#!/usr/bin/env python3
"""
WebSocket vs HTTP SSE Performance Comparison.

Compares Time-To-First-Token (TTFT) latency between:
- HTTP SSE: Creates fresh SDK client per request with session resumption
- WebSocket: Maintains persistent SDK connection in single async context

Expected results:
- WebSocket is ~59% faster for follow-up messages
- WebSocket adds ~10% overhead vs direct SDK
- HTTP SSE adds ~222% overhead vs direct SDK
...
Usage:
    ...
    # Run full comparison:
    python tests/test_websocket_timing.py

    # Run WebSocket-only test:
    python tests/test_websocket_timing.py --websocket-only

    # Run HTTP SSE-only test:
    python tests/test_websocket_timing.py --http-only
"""
import argparse
import asyncio
import json
...
async def test_websocket(num_turns: int = 3) -> list[float]:
    ...
    async with websockets.connect(f'{WS_BASE}/api/v1/ws/chat?agent_id={agent_id}') as ws:
...
def print_summary(http_times: list[float], ws_times: list[float]):
