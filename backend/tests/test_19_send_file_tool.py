...
class TestTryDeliverMediaFile:
    """Test _try_deliver_media_file worker function."""
    ...
    @pytest.mark.asyncio
    async def test_delivers_tts_audio(self):
        """Delivers audio file from TTS result."""
        from platforms.worker import _try_deliver_media_file

        test_file = self.output_dir / "tts_123.mp3"
        test_file.write_bytes(b"\x00" * 1000)

        result_json = json.dumps({
            "audio_path": "test-session/output/tts_123.mp3",
            "download_url": "https://example.com/dl/token",
            "format": "mp3",
        })

        await _try_deliver_media_file(
            "synthesize_speech", result_json, self.session_cwd,
            self.adapter, "chat123", "testuser", "cwd123", self.send_msg_fn,
        )
    ...
    @pytest.mark.asyncio
    async def test_delivers_ocr_output(self):
        """Delivers text file from OCR result."""
        from platforms.worker import _try_deliver_media_file

        test_file = self.output_dir / "ocr_456.txt"
        test_file.write_text("Extracted text content")

        result_json = json.dumps({
            "output_path": "test-session/output/ocr_456.txt",
            "download_url": "https://example.com/dl/token",
        })

        await _try_deliver_media_file(
            "perform_ocr", result_json, self.session_cwd,
            self.adapter, "chat123", "testuser", "cwd123", self.send_msg_fn,
        )
    ...
    @pytest.mark.asyncio
    async def test_ignores_non_media_tool(self):
        """Does nothing for non-media tools."""
        from platforms.worker import _try_deliver_media_file

        await _try_deliver_media_file(
            "Read", '{"audio_path": "x"}', self.session_cwd,
            self.adapter, "chat123", "testuser", "cwd123", self.send_msg_fn,
        )
    ...
    async def test_handles_missing_file(self):
        """Silently handles missing output file (file doesn't exist)."""
        from platforms.worker import _try_deliver_media_file

        result_json = json.dumps({
            "audio_path": "test-session/output/nonexistent.mp3",
        })
        ...
        # _deliver_file_to_platform checks path.exists() and returns early
        # So send_file should not be called (or called but no file sent)
        # The function doesn't error out — it's a silent no-op

    @pytest.mark.asyncio
    async def test_handles_invalid_json(self):
        """Silently handles invalid JSON."""
        from platforms.worker import _try_deliver_media_file

        await _try_deliver_media_file(
            "synthesize_speech", "not json", self.session_cwd,
            self.adapter, "chat123", "testuser", "cwd123", self.send_msg_fn,
        )
