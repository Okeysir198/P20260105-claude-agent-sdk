...
def load_prompt(agent_name: str, version: str | None = None) -> str:
    """
    Load a prompt template by agent name and version.

    Args:
        agent_name: Agent identifier (e.g., "prompt01_introduction")
    ...
    Examples:
        >>> load_prompt("prompt01_introduction")
        "You are {{ agent_name }}..."

        >>> load_prompt("prompt01_introduction", "v2")
    ...
    if version is None:
        version = versions_data.get("defaults", {}).get(agent_name, "v1")

    # Build filename
    if version == "v1":
        filename = f"{agent_name}.yaml"
    else:
        filename = f"{agent_name}_{version}.yaml"
    ...
    if isinstance(data, dict):
        return data.get("prompt") or data.get("instructions") or ""
...
def load_instruction(source: str, version: str | None = None) -> str:
    ...
    - File paths: "prompts/prompt01_introduction.yaml"
    - Agent names: "prompt01_introduction"
    ...
    Returns:
        Raw template string from the prompt file.
    """
    # Handle file path format (e.g., "prompts/prompt01_introduction.yaml")
    if source.endswith(".yaml") or source.endswith(".yml"):
        # Extract agent name from path
        filename = Path(source).stem  # "prompt01_introduction"
